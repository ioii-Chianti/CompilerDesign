...
int SetParameters(char * functor) {
    int i, j, idx, num_args;
    idx = LookUpSymbol(functor);

    num_args = currentCounter - idx - 1;
    Table[idx].args = num_args;
    for (i = currentCounter - 1, j = num_args; i > idx; i--, j--) {
        Table[i].scope = currentScope;
        Table[i].offset = j;
        Table[i].mode = ARGUMENT;
    }
    return idx;
}
